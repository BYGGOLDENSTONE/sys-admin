...
const STALL_HINTS: Dictionary = {
	...
	9: ["Recover Corrupted Blueprint with the Recoverer — same process you used on Financial. Tougher files may need processed fuel"],
	10: ["Decrypt Financial first (Decryptor + Key), then re-encrypt it (Encryptor + Key). Your Key forge and Financial source do the work"],
	...
	14: ["Research appears in Libraries and Hospitals. Use Classifiers to filter it out of mixed sources", "Standard data flows from Vending Machines, Traffic Cameras, and other easy sources — your earliest pipelines still work"],
